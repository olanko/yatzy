defmodule YatzyWeb.Layouts do
  ...
  attr :current_user, :any, default: nil, doc: "currently signed-in user"
    ...
    <header class="flex items-center justify-end gap-2 px-4 pt-4 sm:px-6 lg:px-8">
      ...
      <%= if @current_user do %>
        ...
        <.link
          ...
          title={"Omat asetukset (#{@current_user.username})"}
        >
          <.icon name="hero-cog-6-tooth" class="size-5" />
          <span class="hidden sm:inline">Omat asetukset</span>
        </.link>
        <.link href={~p"/logout"} method="delete" class="btn btn-sm btn-ghost">
          Kirjaudu ulos
        </.link>
      <% else %>
        <.link href={~p"/login"} class="btn btn-sm btn-ghost">
          Kirjaudu sisään
        </.link>
      <% end %>
      <.theme_toggle />
